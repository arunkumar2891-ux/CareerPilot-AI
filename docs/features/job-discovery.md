...
### Paste JD (single job)
```
ApplicationPackageWizard (step 1) → services.jobSearch.pasteJd()
...
- `src/components/jobs/ApplicationPackageWizard.tsx` — 4-step wizard (paste JD → score → generate → results); also the manual JD paste entry point
