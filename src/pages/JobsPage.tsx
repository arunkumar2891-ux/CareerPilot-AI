...
import {
  ...
  Table as TableIcon, Zap, ExternalLink, Copy, FileText, SearchX, Trash2, Cloud, Link2, Gauge, Plus,
...
export function JobsPage() {
  ...
  const filtered = filterJobs(jobs, { search, ...filters });
  ...
  const clearAllJobs = async () => {
    ...
    try {
      ...
      await invalidateAll(qc, ['jobs', 'applications', 'metrics']);
  ...
  const repairSync = async () => {
    ...
    try {
      ...
      await invalidateAll(qc, ['jobs', 'resumes', 'integrations']);
  ...
  const applyableJobs = selectApplyableJobs(jobs, selectedJobIds);

  const extractableJobs = selectExtractableJobs(jobs, selectedJobIds);
